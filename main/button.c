...
#define PERMIT_JOIN_S                                                          \
  ROUTER_PERMIT_JOIN_DURATION_S   /* permit-join window duration         */
...
static void do_permit_join(void) {
  ...
  xTimerChangePeriod(s_pj_timer, pdMS_TO_TICKS((uint32_t)PERMIT_JOIN_S * 1000U),
                     0);
...
static void do_tx_toggle(void) {
  ...
  int8_t pwr =
      s_high_power ? ROUTER_TX_POWER_HIGH_DBM : ROUTER_TX_POWER_LOW_DBM;
  ...
  ESP_LOGI(TAG, "TX power -> %d dBm (%s)", pwr,
           s_high_power ? "BOOST (20 dBm)" : "NORMAL (8 dBm)");
...
static void start_blink(bool fast, uint8_t r, uint8_t g, uint8_t b) {
  ...
  s_blink_state = false;
  TickType_t period =
      fast ? pdMS_TO_TICKS(BLINK_FAST_MS) : pdMS_TO_TICKS(BLINK_SLOW_MS);
 ...
 */
bool button_is_night_mode(void) { return s_night_mode; }
/**
...
esp_err_t button_init(void) {
  ...
  ESP_RETURN_ON_ERROR(gpio_isr_handler_add(BOOT_BTN_GPIO, btn_isr, NULL), TAG,
                      "gpio_isr_handler_add failed");

  s_tap_timer = xTimerCreate("btn_tap", pdMS_TO_TICKS(TAP_WINDOW_MS), pdFALSE,
                             NULL, tap_window_cb);
  if (!s_tap_timer)
    return ESP_ERR_NO_MEM;

  s_hold_timer = xTimerCreate("btn_hold", pdMS_TO_TICKS(HOLD_5S_MS), pdFALSE,
                              NULL, hold_cb);
  if (!s_hold_timer)
    return ESP_ERR_NO_MEM;

  s_blink_timer = xTimerCreate("btn_blink", pdMS_TO_TICKS(BLINK_SLOW_MS),
                               pdTRUE, NULL, blink_cb);
  if (!s_blink_timer)
    return ESP_ERR_NO_MEM;

  s_pj_timer =
      xTimerCreate("btn_pj", pdMS_TO_TICKS((uint32_t)PERMIT_JOIN_S * 1000U),
