...
typedef struct esp_zb_ota_file_parser_s {
  uint32_t offset; /*!< Byte offset within current element header/value being
                      parsed. Used to track partial header across multiple data
                      blocks. Reset to 0 when starting a new element. */
   ...
   */
  struct {
    uint16_t type;   /*!< Element Type field (2 bytes): Tag identifier (e.g.,
