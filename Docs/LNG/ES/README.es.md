...
| `main/button.c` | Gestos del botón BOOT (GPIO9): ISR + timers FreeRTOS, toggle TX, factory reset, permit-join |
...
El botón BOOT del DevKitC-1 es un GPIO de propósito general una vez que el firmware está en ejecución. El módulo `button.c` instala una ISR en `ANYEDGE` y gestiona **cuatro** software timers FreeRTOS para detectar los siguientes gestos:

| Gesto | Umbral | Acción | Feedback LED |
|-------|--------|--------|-------------|
| Doble toque | < 500 ms entre pulsaciones | Abre ventana Permit Join durante `ROUTER_PERMIT_JOIN_DURATION_S` (60 s) | 🩵 Cian pulsante mientras la ventana está abierta |
...
| 🩵 Cian pulsante | 0 | 64 | 64 | Ventana Permit Join abierta localmente (doble-tap) |
...
- 🩵 **Cian pulsante** — ventana Permit Join abierta localmente (doble-tap); pueden unirse nuevos dispositivos
...
|---------|-------|---------|
| **v4.0.0** | 2026-06-21 | Gesto doble-tap Permit Join, LED cian pulsante, atributo ZCL `SWBuildID`. |
